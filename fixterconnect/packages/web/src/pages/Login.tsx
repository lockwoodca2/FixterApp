...
const Login: React.FC = () => {
  ...
  const handleGoogleSuccess = async (credentialResponse: any) => {
    ...
    try {
      const { user, isNewUser } = await loginWithGoogle(credentialResponse.credential);

      if (user) {
        if (isNewUser) {
          // New user - might want to show account type selection
          // For now, default to client and redirect
          navigate(user.type === 'contractor' ? '/contractor-dashboard' : '/client-dashboard');
        } else {
  ...
  const handleFacebookLogin = () => {
    ...
    alert('Facebook login coming soon! For now, use Google or email/password.');
  };
  ...
  const handleSendCode = async () => {
    ...
    setTimeout(() => {
      setCodeSent(true);
      setIsLoading(false);
      alert('Phone authentication coming soon! For now, use Google or email/password.');
    }, 1000);
  };
