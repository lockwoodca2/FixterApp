 ...
 */
export async function getContractorAvailability(contractorId: number, date: Date) {
...
export async function getContractorAvailabilityRange(
  ...
  while (currentDate <= end) {
    const availability = await getContractorAvailability(contractorId, new Date(currentDate));
 ...
 */
export async function canAcceptBooking(contractorId: number, date: Date): Promise<boolean> {
  const availability = await getContractorAvailability(contractorId, date);
...
export async function getNextAvailableDate(
  ...
  for (let i = 0; i < maxDaysAhead; i++) {
    const availability = await getContractorAvailability(contractorId, currentDate);
